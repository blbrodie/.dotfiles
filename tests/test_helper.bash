...
# Source a single bashrc.d module from the repo/worktree this test lives in
# (one level above tests/), NOT the user's installed $HOME/.dotfiles copy.
# This lets tests run correctly inside a git worktree where changes haven't
# been merged yet. Each module is self-contained (defines functions only),
# so sourcing it has no side effects beyond those definitions.
source_bashrc_module() {
